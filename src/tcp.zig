...
pub const Header = struct {
    ...
    pub fn parse(bytes: []const u8) !Header {
        ...
        const cwr: bool = @bitCast(try reader.readBitsNoEof(u1, 1));
        const ece: bool = @bitCast(try reader.readBitsNoEof(u1, 1));
        const urg: bool = @bitCast(try reader.readBitsNoEof(u1, 1));
        const ack: bool = @bitCast(try reader.readBitsNoEof(u1, 1));
        const psh: bool = @bitCast(try reader.readBitsNoEof(u1, 1));
        const rst: bool = @bitCast(try reader.readBitsNoEof(u1, 1));
        const syn: bool = @bitCast(try reader.readBitsNoEof(u1, 1));
        const fin: bool = @bitCast(try reader.readBitsNoEof(u1, 1));
